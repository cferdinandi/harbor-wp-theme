...
(function (root, factory) {
	...
	// Default settings
	var defaults = {
		selectorContainer: '[data-petfinder-img-container]',
		selectorImg: '[data-petfinder-img]',
		selectorToggle: '[data-petfinder-img-toggle]',
		initClass: 'js-petfinder-images',
		imgAttributes: '',
		callback: function () {}
	};
	...
	var toggleImage = function ( toggle ) {
		...
		// Variables
		var container = getClosest( toggle, settings.selectorContainer );
		if ( !container ) return;
		var img = container.querySelector( settings.selectorImg );
		if ( !img ) return;

		// Load image
		img.innerHTML = '<img ' + settings.imgAttributes + ' src="' + toggle.href + '">';
	...
	var eventHandler = function (event) {
		...
		var closest = getClosest( toggle, settings.selectorToggle );
		if ( closest ) {
